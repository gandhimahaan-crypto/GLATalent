...
export function AdminDashboard() {
  ...
  return (
    <PageWrapper sidebar="admin">
      <div className="page-head"><h1>Admin Overview</h1></div>
      <section className="grid four">{adminMetrics.map(([value, label]) => <Card className="metric" key={label}><h2>{value}</h2><p>{label}</p></Card>)}</section>
      <section className="grid two"><Card><h2>Institution Domain Demand</h2>{stats?.domains?.length ? <MonoBarChart data={stats.domains} xKey="name" yKey="value" /> : <p>No data available yet.</p>}</Card><Card><h2>System Readiness Distribution</h2>{stats?.readinessDistribution?.length ? <MonoBarChart data={stats.readinessDistribution} xKey="range" yKey="students" /> : <p>No data available yet.</p>}</Card></section>
      <Card><h2>System At-Risk Overview</h2>{rows.length ? <><Table columns={["Rank", "Student Name", "Roll No.", "CGPA", "Readiness Score", "Predicted Domain", "Risk Level", "Action"]} rows={rows} /><div className="pagination">Prev <strong>1</strong> 2 3 Next</div></> : <p>No data available yet.</p>}</Card>
