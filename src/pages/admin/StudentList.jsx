...
export function StudentList() {
  ...
  const rows = students.map((student) => ({
    id: student.id, Name: student.name, "Roll No.": student.rollNo, Branch: student.branch, CGPA: student.cgpa,
  ...
  return (
    <PageWrapper sidebar="admin">
      ...
      <Card>{rows.length ? <><Table columns={["Name", "Roll No.", "Branch", "CGPA", "Readiness Score", "Predicted Domain", "Status"]} rows={rows} onRowClick={(row) => navigate(`/admin/students/${row.id}`)} /><div className="pagination">Prev <strong>1</strong> 2 3 Next</div></> : <p>No students found.</p>}</Card>
    </PageWrapper>
  );
}
